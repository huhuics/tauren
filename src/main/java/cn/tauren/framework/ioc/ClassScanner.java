...
public final class ClassScanner {
    ...
    public static List<Class<?>> getClasses(String pkgName) {
        ...
        try {
            ...
            throw new RuntimeException("扫面类失败", e);
        }
     ...
     * @param classes  类的集合
     */
    private static void addClassesToList(String pkgName, String pkgPath, List<Class<?>> classes) {
        ...
        File[] files = pkgFile.listFiles(new FileFilter() {
            ...
            public boolean accept(File file) {
                return file.isDirectory() || file.getName().startsWith(".class");
            }
        });

        for (File file : files) {
            if (file.isDirectory()) { //如果是目录则递归遍历
                String subPkgName = pkgName + FILE_DOT + file.getName();
                addClassesToList(subPkgName, file.getAbsolutePath(), classes);
            } else {//处理class文件
                String className = file.getName().substring(0, (int) file.length() - 6);//去除.class后缀
                String classFullName = pkgName + FILE_DOT + className;
                try {
                    classes.add(Class.forName(classFullName, false, Thread.currentThread().getContextClassLoader()));
